...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-24 relative overflow-hidden">
      {/* Glow effects */}
      <div className="absolute right-0 top-1/4 w-[350px] h-[350px] rounded-full glow-orb-emerald -z-10 animate-float-medium"></div>

      <div className="max-w-7xl mx-auto px-6">
        {/* Section Heading */}
        <div className="text-center mb-16">
          <h2 className="font-sora font-extrabold text-3xl md:text-4xl text-slate-800 dark:text-white mb-4">
            Skills &amp; <span className="bg-gradient-to-r from-emerald-500 to-teal-600 bg-clip-text text-transparent">Tools</span>
          </h2>
            ...
            {filteredSkills.map((skill) => (
              <motion.div
                ...
                className={`p-6 rounded-3xl transition-all duration-300 group hover-lift relative ${skill.highlighted
                    ? 'glass-panel-strong border-emerald-500/30 shadow-glass-emerald'
                    : 'glass-panel border-white/20 dark:border-white/5'
                  }`}
              >
                {/* Glow border on hover */}
                <div className="absolute inset-0 rounded-3xl bg-gradient-to-tr from-emerald-500 to-teal-500 opacity-0 group-hover:opacity-10 transition-opacity duration-300 -z-10" />

                <div className="flex justify-between items-start mb-4">
                  <div className={`p-3 rounded-2xl ${skill.highlighted ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'} group-hover:scale-110 transition-transform`}>
                    {skill.icon}
                  </div>
                  {skill.badge && (
                  ...
                  {skill.name === 'React.JS' && (
                    <span className="px-2.5 py-0.5 rounded-full bg-emerald-500 text-white text-[10px] font-sora font-extrabold uppercase tracking-wide animate-pulse">
                      Core
                    </span>
                  )}
                </div>

                <h3 className="font-sora font-bold text-lg text-slate-800 dark:text-white mb-2 group-hover:text-emerald-500 dark:group-hover:text-emerald-400 transition-colors">
                  {skill.name}
                </h3>
